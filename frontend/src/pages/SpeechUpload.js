    ...
    useEffect(() => {
        api.get('/speech/tasks').then(({ data }) => {
            ...
            if (data.tasks.length)
                setTask(data.tasks[Math.floor(Math.random() * data.tasks.length)].id);
        }).catch(() => {
        ...
        if (id)
            api.get(`/speech/session/${id}`).then(({ data }) => {
                ...
                if (options.length) {
                    const assignedTasks = options.map((item, index) => ({ id: index === 0 ? 'reading' : `reading_${index + 1}`, title: `Passage ${index + 1}`, prompt: item.paragraph }));
                    setTasks(assignedTasks);
                    setTask(assignedTasks[Math.floor(Math.random() * assignedTasks.length)].id);
                }
                else if (data.speech_set?.paragraph)
