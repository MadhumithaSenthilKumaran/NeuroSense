...
type SpeechFeature = { _id: string; transcript?: string | null; duration_s: number; pitch_hz?: number | null; pause_duration_s: number; speech_rate_wpm?: number | null; reading_accuracy?: number; accuracy_rate?: number; content_match?: { score?: number | null }; transcript_word_count?: number; expected_word_count?: number }
...
export default function SpeechUpload() {
  ...
  return <div className="speech-page">
    ...
    {result && <section className="results-panel"><div><div className="panel-kicker">03 / Results ready</div><h2>Speech signal captured</h2><p className="result-note">Your acoustic profile has been added to this assessment.</p></div><div className="metric-row"><Metric label="Duration" value={`${result.duration_s}s`} /><Metric label="Pitch" value={`${result.pitch_hz || 0} Hz`} /><Metric label="Pause time" value={`${result.pause_duration_s}s`} /><Metric label="Speech rate" value={`${result.speech_rate_wpm || 0} WPM`} /><Metric label="Reading accuracy" value={`${result.reading_accuracy ?? result.accuracy_rate ?? 0}%`} /><Metric label="Word coverage" value={`${result.transcript_word_count || 0}/${result.expected_word_count || 0}`} /></div><div className="transcript"><span>Transcript</span><p>{result.transcript || 'No transcript detected in the recording.'}</p></div><Link className="continue-action" to={`/assessment/finalize/${id}`}>Continue to final review <span>-&gt;</span></Link></section>}
  </div>
}
