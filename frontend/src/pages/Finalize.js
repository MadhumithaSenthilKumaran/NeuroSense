...
function CognitiveBreakdown({ breakdown }) {
    return _jsxs("section", { className: "recommendation-panel cognitive-breakdown", children: [_jsx("h3", { children: "How the cognitive score was formed" }), _jsx("p", { children: breakdown.method }), _jsx("div", { className: "recommendation-grid", children: breakdown.parts.map((part) => _jsxs("div", { className: "recommendation-item", children: [_jsxs("strong", { children: [part.name, " \u00B7 ", part.weight_percent, "%"] }), _jsx("span", { children: part.score == null ? 'Not recorded' : `${part.score}/100` }), _jsx("small", { children: part.contribution_points == null ? 'No contribution' : `${part.contribution_points} points contributed` }), _jsx("small", { children: part.explanation }), part.key === 'visual_memory' && part.game_breakdown?.number_elapsed_ms != null && _jsxs("small", { children: ["Number-order time: ", (Number(part.game_breakdown.number_elapsed_ms) / 1000).toFixed(2), "s \u00B7 sequence completed: ", part.game_breakdown.number_sequence_completed ? 'yes' : 'no', "."] }), part.key === 'praxis_camera' && _jsxs(_Fragment, { children: [_jsxs("small", { children: ["Sequential camera cross-check: ", part.game_breakdown?.camera_correct_trials ?? 0, "/", part.game_breakdown?.camera_total_trials ?? 0, " numbers correct (", part.game_breakdown?.camera_sequence_score ?? 0, "/100)."] }), part.game_breakdown?.praxis_trials?.map((trial) => _jsxs("small", { children: ["Number ", trial.prompted_number, ": detected ", trial.gestured_number_detected, "; latency ", trial.metrics?.motor_latency_seconds ?? 'n/a', "s, frames with hand ", trial.metrics?.frames_with_hand ?? 0, "."] }, `${trial.prompted_number}-${trial.gestured_number_detected}`))] })] }, part.key)) })] });
}
